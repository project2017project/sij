					...
					<div class="mr-table allproduct">
                        @include('includes.admin.form-success')  
							<div class="table-responsiv">
							    <form id="geniusform" class="form-horizontal" action="{{route('admin-prod-bulkstock-update')}}" method="POST" enctype="multipart/form-data">      
    						{{ csrf_field() }}
								<table id="geniustable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
									<thead>
    									<tr>
					              <th>{{ __("Name") }}</th><th>{{ __("Type") }}</th><th>{{ __("Stock") }}</th><th>{{ __("Price") }}</th><th>{{ __("Regular Price") }}</th><th>{{ __("Update By") }}</th><th>{{ __("Status") }}</th><th>{{ __("Options") }}</th>
										</tr>
									</thead>
								</table>
...
{{-- DATA TABLE --}}
    <script type="text/javascript">
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-prod-productupdate-datatables') }}',
               columns: [
                        { data: 'name', name: 'name' },
                        { data: 'type', name: 'type' },
