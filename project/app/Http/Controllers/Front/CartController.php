...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        $random_products =  Product::where('status','=',1)->select($selectable)->inRandomOrder()->take(2)->get();
        return view('front.cart', compact('products','totalPrice','mainTotal','tx','random_products')); 
    }
   ...
   public function addcart($id)
    ...
    public function addnumcart()
    {
        ...
        $prod = Product::where('id','=',$id)->first(['id','user_id','slug','name','photo','size','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes']);
         ...
         if($gs->store_product_sale==1){
            $prc = $prod->price-$prod->price * $gs->store_product_sale_amount/100;    
        }else{
            $prc = $prod->price ;
        }
    ...
    public function couponcheck()
    {
        ...
        $coupon = Coupon::where('code','=',$code)->first();
		  if($coupon->gift_voucher == 1 && $coupon->used == 1)
        { 
	     return response()->json(0);
	    }else{ 
            if (Session::has('currency')) 
