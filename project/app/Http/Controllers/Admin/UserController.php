...
class UserController extends Controller
{
    ...
    {
        $data = User::findOrFail($id);
        return view('admin.user.changepwd',compact('data'));
    }
...
// OTHER SECTION ENDS
        ...
        public function accept($id)
        {
			...
			$order = Order::find($withdraw->group_order_id);
            //--- Redirect Section     
			...
			$gs = Generalsetting::findOrFail(1);
                 if($gs->is_smtp == 1){
                    $maildata = [
                        ...
                        'subject' => 'Your order '.$order->order_number.' Withdraw Accepted Successfully.',
        ...
        public function reject($id)
        {
			...
			$gs = Generalsetting::findOrFail(1);
                 if($gs->is_smtp == 1){
                    $maildata = [
                        'to' => $order->customer_email,
                        'subject' => 'Your order '.$order->order_number.' Withdraw Rejected Successfully.',
                        'body' => "Hello ".$order->customer_name.","."\n Thank you for shopping with us. We are looking forward to your next visit.",
                    ];
