      ...
      <div class="category-module hiraola-sidebar_categories">
        ...
        <div class="module-body">          
          <ul  class="module-list_item">
            ...
            <li>
              <a href="{{route('front.onsale', $element->slug)}}"> <i class="icon licon-chevron-right"></i>  {{$element->name}} <span class="notranslate"></span> </a>
              @if(!empty($cat) && $cat->id == $element->id && !empty($cat->subs))
              <ul class="module-sub-list_item">
                @foreach ($cat->subs as $key => $subelement)
                <li>
                  <a href="{{route('front.onsale', [$cat->slug, $subelement->slug])}}" ><i class="icon licon-chevron-right"></i> {{$subelement->name}} <span class="notranslate"></span></a>
                  @if(!empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs))
                  <ul  class="module-list_item">
                    @foreach ($subcat->childs as $key => $childcat)
                    <li>
                      <a href="{{route('front.onsale', [$cat->slug, $subelement->slug, $childcat->slug])}}"><i class="icon licon-chevron-right"></i>  {{$childcat->name}} <span class="notranslate"></span></a>
                    </li>
                    @endforeach
                  </ul>
                  @endif
      ...
      <div class="hiraola-sidebar-catagories_area" style="display:none">
        <div class="hiraola-sidebar_categories">
          ...
          <div class="price-filter">
            <form id="catalogForm" action="{{ route('front.onsale', [Request::route('category'), Request::route('subcategory'), Request::route('childcategory')]) }}" method="GET">
