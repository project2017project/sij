...
class OnsaleController extends Controller
{
   ...
   public function index(Request $request, $slug=null, $slug1=null, $slug2=null)
	...
	foreach ($product_data as $key => $product_val) {
		...
		$or_price = $product_val->price;
		$pre_price = $product_val->previous_price;
		if(@$product_val->size_price){
		$vor_price = $product_val->size_price;
		}
		if(@$product_val->size_pre_price){
		$vpre_price = $product_val->size_pre_price;
		}        
		...
		if($or_price && $pre_price) {
		if($pre_price > $or_price){
			$sal_status =1;
		}else{
			$sal_status =0;
		}
		}else if($vor_price && $vpre_price) {
		if($vpre_price > $vor_price){
			$sal_status =1;
		}else{
			$sal_status =0;
		}
		...
		$product_up = Product::where('id','=',$product_val->id)->update(['sum_stock' => $total_stock,'sal_status' => $sal_status]);
	}
	}
        $this->code_image();
	...
	$prods = $prods->where('sum_stock','!=','');    
	$prods = $prods->where('sal_status',1)->get();
	
	
    

        $vprods = (new Collection(Product::filterProducts($prods)))->where('sum_stock','>=',1)->paginate(18);
        $data['vprods'] = $vprods;
        ...
        if($request->ajax()) {
          ...
          return view('includes.product.filtered-saleproducts', $data);
    }
        return view('front.onsale', $data);
    }     
