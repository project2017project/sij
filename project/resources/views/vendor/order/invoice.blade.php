            ...
            <tr class="top">
                <td colspan="2">
                    <table>
                        <tr>
                            <td class="title">
                                <span class="t-invoice">{{ __('INVOICE') }} <a class="btn  add-newProduct-btn print" href="{{route('vendor-order-print',$order->order_number)}}"
                        target="_blank"><i class="fa fa-print"></i> {{ $langg->lang607 }}</a></span><br><br>
...
<p><b>>@if(!empty($product['item']['variation_title'])){{ $product['item']['variation_title'] }} @else {{ $langg->lang312 }} @endif : </b> {{str_replace('-',' ',$product['size'])}} </p>
@endif</span> <br />
                ...
                <td>
                    <span class="t-payment-method"> @php
                                            ...
                                            @endphp 
											{{ App\Models\Currency::where('sign',$order->inr_currency_sign)->first()->name }} {{ round($total, 2) }}
											 @php
...
@endif
        ...
        <table class="invoice-items" cellpadding="0" cellspacing="0">    
            <tr class="heading">
                ...
                <td><span class="t-price">Subtotal : &nbsp; &nbsp; &nbsp; {{ App\Models\Currency::where('sign',$order->inr_currency_sign)->first()->name }} {{ round($subtotal, 2) }}</span></td>
            </tr>
        </table>
