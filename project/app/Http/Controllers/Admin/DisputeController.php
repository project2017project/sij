...
class DisputeController extends Controller
{
    ...
    public function index(){
        ...
        return view('admin.dispute.index',compact('users'));
    } 
	 public function disload($id)
    {
        $order = Order::whereRaw("find_in_set($id , vendor_id_list)")->orderBy('id','desc')->get();
        return view('admin.dispute.order',compact('order'));
    }
	 public function dispload($id)
    {
        $product = Product::where('id','=',$id)->orderBy('id','desc')->get();
        return view('admin.dispute.product',compact('product'));
    }
        ...
        if ($validator->fails()) {
          return response()->json(array('errors' => $validator->getMessageBag()->toArray()));
            ...
            $vendor_id = $request->vendor_id;			
	         return response()->json(array('redirect_url' => route('admin-order-adisputes',['orderid'=>$order_id,'vendorid'=>$vendor_id])));	 
    }
	public function add($orderid,$vendorid)
    {
		
		$vdata = VendorOrder::where('order_id','=',$orderid)->where('user_id','=',$vendorid)->orderBy('id','desc')->get();
		return view('admin.dispute.add',compact('vdata','orderid','vendorid'));
	}
	public function addstore(Request $request)
    {
        ...
        if ($validator->fails()) {
          return response()->json(array('errors' => $validator->getMessageBag()->toArray()));
			...
			$users_id=$Dispute->user_id;
			$user_id=$Dispute->vendor_id;
			$product_id=$Dispute->product_id;
			$product_quantity=$data_vorder->qty;
			$product_amount=$data_vorder->price;
			$product_data = Product::findOrFail($product_id);
			$newmsg ='Your Order Dispute Request Created.';
            ...
            $admin_subject = 'South India Jewels order '.$order->order_number.' Dispute Request Created';			
			$to = $order->customer_email;           			
            $subject = 'Your South India Jewels order '.$order->order_number.' Dispute Request Created';
...
$vendor_subject = 'Dispute Request For Your South India Jewels order '.$order->order_number.'';
                                 ...
                                 <tbody>
                                    <tr>
                                       <td style="padding:36px 48px;display:block">
                                          <h1 style="font-size:30px;font-weight:300;line-height:150%;margin:0;text-align:left;color:#ffffff"><span class="il"></span> <span class="il">Request Dispute Order</span>: #'.$order_number.'</h1>
                                       </td>
                                    </tr>
                                 </tbody>
                              </table>
                                                ...
                                                <tr>
                                                   <td valign="top" style="padding:48px 48px 32px">
                                                      <div style="color:#717a7a;font-size:14px;line-height:150%;text-align:left">
                                                         ...
                                                         <p style="margin:0 0 16px">We have reuested the Dispute for your order #'.$order_number.'. It will be credited in 5-7 business working days.</p>
...
$objDemo = new \stdClass();
		...
		mail($admin_email, $admin_subject, $admin_msg, $headers);
			...
			return response()->json(array('redirect_url' => route('admin-dispute-show',$Dispute->id)));
			}
    }
	
	public function opendispute()
    {
		return view('admin.dispute.open');
	}
	
	public function opendispute_datatables()
    {
         
		$datas = Dispute::where('status','=',0)->orderBy('id','desc')->get();     

         //--- Integrating This Collection Into Datatables
         return Datatables::of($datas)
		                      ->editColumn('id',function(Dispute $data){
                                $id = $data->id;
                                return  $id;
                            }) 	
		                     ->editColumn('vendor_id',function(Dispute $data){
                                $users = User::find($data->vendor_id);
                                return  $users->name;
                            }) 
                            ->editColumn('order_id',function(Dispute $data){
                                $order_id = '<a href="'.route('admin-order-show',$data->order_id).'">'.$data->order_id.'</a>';
                                return $order_id; 
                            }) 
                           ->editColumn('product_name',function(Dispute $data){
                                $product_name = $data->product_name;
                                return  $product_name;
                            })  
                          ->editColumn('product_sku',function(Dispute $data){
                                $product_sku = $data->product_sku;
                                return  $product_sku;
                            }) 							 
                           ->editColumn('created_at', function(Dispute $data) {
								return $data->created_at;
                            }) 
                       ->editColumn('action', function(Dispute $data) {								
								$action = '<div class="godropdown"><button class="go-dropdown-toggle"> Actions<i class="fas fa-chevron-down"></i></button>
                                <div class="action-list"><a href="'.route('admin-disputeds-show',$data->id).'"><i class="fa fa-eye"></i>Details</a>
									 </div>
                                </div>';
								return $action;
	...
	public function ustatus(Request $request, $id)
    {
	    ...
	    $product_id=$dispute->product_id;
		$data_vorder = VendorOrder::where(['user_id' => $dispute->vendor_id,'order_id' => $dispute->order_id,'product_id' => $dispute->product_id])->first();
        $product_quantity=$data_vorder->qty;		
        $dispute->status = 1;
        $dispute->update();
		$product_data = Product::findOrFail($product_id);
		$newmsg = 'Dispute Status Update'; 
        $newmsg .= '<br> Product Name : ' . $product_data->name.'( '.$product_data->sku.' )';		
                                             ...
                                             <tbody>
                                                <tr>
                                                   <td valign="top" style="padding:48px 48px 32px">
                                                      <div style="color:#717a7a;font-size:14px;line-height:150%;text-align:left">
                                                         <p style="margin:0 0 16px">Hi '.$user_info->name.',</p>
                                                         <p style="margin:0 0 16px">We have initiated the Dispute for your order #'.$order_number.'.</p>
                                                         <h2 style="color:#111111;display:block;font-size:18px;font-weight:bold;line-height:130%;margin:0 0 18px;text-align:left">
                                                            <a href="https://shop.southindiajewels.com/demo/wp-admin/post.php?post=10439&amp;action=edit" style="font-weight:normal;text-decoration:underline;color:#111111" target="_blank">[<span class="il">Order</span> #'.$order_number.']</a> ('.date('M d,Y',strtotime($order->created_at)).')
                                                         </h2>
                                                         <div style="margin-bottom:40px">
...
if($prod['size']){
   ...
   if($gs->is_smtp == 1)
    ...
    {
		return view('admin.dispute.resolved');
	}
	public function resolved_datatables()
    {
         
		$datas = Dispute::where('status','=',1)->orderBy('id','desc')->get();        
	...
	public function show($id)
    {
		...
		return view('admin.dispute.details',compact('data'));
	}
	public function alldisputeExcelFile(Request $request){
	$all_datas = Dispute::orderBy('id','desc')->get();
	...
	$headers = array(
		...
		$columns = array('SL No','Vendor Name','Order Id','Product Name','Product SKU','Dispute Date');
        ...
        $file = fopen('php://output', 'w');
         fputcsv($file, $columns);		 
            foreach ($all_datas as $all_data) {	
			  ...
			  $row['Dispute Date']    = $created_at;			  
			  fputcsv($file, array($row['SL No'],$row['Vendor Name'],$row['Order Id'],$row['Product Name'],$row['Product SKU'],$row['Dispute Date'] ));
				$j++;
			}			
            fclose($file);
        };	
