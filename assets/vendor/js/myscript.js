...
// **************************************  AJAX REQUESTS SECTION *****************************************
  ...
  // Display Subcategories Ends
   ...
   $(document).ready(function() {
    $("form#geniusformdata, form#geniusform, form#prodbulkform, form#exshipform, form#rtoshipform,form#messageform,form#verifyform,form#trackform").submit(function() {
...
// Quick Edit Vendor OPERATION

$(document).on('click','.quickeditvendor',function(){
if(admin_loader == 1)
  {
  $('.submit-loader').show();
}
...
$(document).on('submit','#prodbulkform',function(e){
e.preventDefault();
if(admin_loader == 1)
  {
$('.gocover').show();
...
var fd = new FormData(this);
var prodbulkform = $(this);
$('button.prodbulkform-btn').prop('disabled',true);
    $.ajax({
     method:"POST",
     url:$(this).prop('action'),
     data:fd,
     ...
     success:function(data)
     {
        ...
        if ((data.errors)) {
        prodbulkform.parent().find('.alert-success').hide();
        prodbulkform.parent().find('.alert-danger').show();
        prodbulkform.parent().find('.alert-danger ul').html('');
          for(var error in data.errors)
          {
            $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
          }
          prodbulkform.find('input , select , textarea').eq(1).focus();       
        
        }
        else
        {
          prodbulkform.parent().find('.alert-danger').hide();
          prodbulkform.parent().find('.alert-success').show();
          prodbulkform.parent().find('.alert-success p').html(data);
          prodbulkform.find('input , select , textarea').eq(1).focus();
		  setTimeout(function(){
        ...
        $('button.prodbulkform-btn').prop('disabled',false);
...
$(document).on('submit','#exshipform',function(e){
e.preventDefault();
if(admin_loader == 1)
  {
$('.gocover').show();
...
var fd = new FormData(this);
var exshipform = $(this);
$('button.exshipform-btn').prop('disabled',true);
    $.ajax({
     method:"POST",
     url:$(this).prop('action'),
     data:fd,
     ...
     success:function(data)
     {
        ...
        if ((data.errors)) {
        exshipform.parent().find('.alert-success').hide();
        exshipform.parent().find('.alert-danger').show();
        exshipform.parent().find('.alert-danger ul').html('');
          for(var error in data.errors)
          {
            $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
          }
          exshipform.find('input , select , textarea').eq(1).focus();       
        
        }
        else
        {
          exshipform.parent().find('.alert-danger').hide();
          exshipform.parent().find('.alert-success').show();
          exshipform.parent().find('.alert-success p').html(data);
          exshipform.find('input , select , textarea').eq(1).focus();
		  setTimeout(function(){
                  location.reload(true);
                }, 1000);
        ...
        $('button.exshipform-btn').prop('disabled',false);
...
$(document).on('submit','#rtoshipform',function(e){
e.preventDefault();
if(admin_loader == 1)
  {
$('.gocover').show();
...
var fd = new FormData(this);
var rtoshipform = $(this);
$('button.rtoshipform-btn').prop('disabled',true);
    $.ajax({
     method:"POST",
     url:$(this).prop('action'),
     data:fd,
     ...
     success:function(data)
     {
        ...
        if ((data.errors)) {
        rtoshipform.parent().find('.alert-success').hide();
        rtoshipform.parent().find('.alert-danger').show();
        rtoshipform.parent().find('.alert-danger ul').html('');
          for(var error in data.errors)
          {
            $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
          }
          rtoshipform.find('input , select , textarea').eq(1).focus();       
        
        }
        else
        {
          rtoshipform.parent().find('.alert-danger').hide();
          rtoshipform.parent().find('.alert-success').show();
          rtoshipform.parent().find('.alert-success p').html(data);
          rtoshipform.find('input , select , textarea').eq(1).focus();
		  setTimeout(function(){
                  location.reload(true);
                }, 1000);
        ...
        $('button.rtoshipform-btn').prop('disabled',false);
