...
class GeneralSettingController extends Controller
{
    ...
    public function generalupdatepayment(Request $request)
    {
        ...
        if ($request->ccavenue_mode == ""){
            $input['ccavenue_mode'] = 'live';
        }
        else {
            $input['ccavenue_mode'] = 'sandbox';
        }
    ...
    public function ccavenue($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_ccavenue = $status;
        $data->update();
    }
    ...
    public function talkto($status)
    {
	 ...
	 public function orderapprove($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->withdraw_approve = $status;
        $data->update();
    }
	 public function approvepermission($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->withdraw_permission = $status;
        $data->update();
    }
    public function webpushr($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_webpushr = $status;
        $data->update();
    }
