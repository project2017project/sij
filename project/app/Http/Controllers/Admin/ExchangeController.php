...
class ExchangeController extends Controller
{
        ...
        if ($validator->fails()) {
          return response()->json(array('errors' => $validator->getMessageBag()->toArray()));
            ...
            $vendor_id = $request->vendor_id;			
	         return response()->json(array('redirect_url' => route('admin-order-aexchange',['orderid'=>$order_id,'vendorid'=>$vendor_id])));	 

    }
	public function add($orderid,$vendorid)
    {
		$vdata = VendorOrder::where('order_id','=',$orderid)->where('user_id','=',$vendorid)->orderBy('id','desc')->get();
		return view('admin.exchange.add',compact('vdata','orderid','vendorid'));
	}
	...
	public function addstore(Request $request)
    {
        ...
        if($files=$request->file('screenshot')){
			...
			$Exchange->fill($input)->save();
			$m_id=$Exchange->id;
			$order_id=$Exchange->order_id;
			$users_id=$Exchange->user_id;
			$user_id=$Exchange->vendor_id;
			$product_id=$Exchange->product_id;
			$product_quantity=$Exchange->quantity;
			$product_amount=$Exchange->amount;
			$product_data = Product::findOrFail($product_id);
			$newmsg ='Your Order Exchange Request Created.';
			$newmsg .= '<br> Product Name : ' . $product_data->name.'( '.$product_data->sku.' )';
			 if(!empty($Exchange->tracking_code)){
            $newmsg .='<br> Code : '. $Exchange->tracking_code;
        }else{
            $newmsg .='';
        }
        if(!empty($Exchange->courier_partner)){    
            $newmsg .='<br>  Company Name : '. $Exchange->courier_partner;
        }else{
            $newmsg .='';
        }
        if(!empty($Exchange->tracking_url)){    
            $newmsg .='<br>  URL : <a href='.$Exchange->tracking_url.' target="_BLANK">'. $Exchange->tracking_url.'</a>';
        }else{
            $newmsg .='';
        }
            ...
            $admin_subject = 'South India Jewels order '.$order->order_number.' Exchange Request Created';			
			$to = $order->customer_email;           			
            $subject = 'Your South India Jewels order '.$order->order_number.' Exchange Request Created';
...
$vendor_subject = 'Exchange Request For Your South India Jewels order '.$order->order_number.'';
                                 ...
                                 <tbody>
                                    <tr>
                                       <td style="padding:36px 48px;display:block">
                                          <h1 style="font-size:30px;font-weight:300;line-height:150%;margin:0;text-align:left;color:#ffffff"><span class="il"></span> <span class="il">Request Exchange Order</span>: #'.$order_number.'</h1>
                                       </td>
                                    </tr>
                                 </tbody>
                              </table>
                                                ...
                                                <tr>
                                                   <td valign="top" style="padding:48px 48px 32px">
                                                      <div style="color:#717a7a;font-size:14px;line-height:150%;text-align:left">
                                                         ...
                                                         <p style="margin:0 0 16px">We have reuested the exchange for your order #'.$order_number.'. It will be credited in 5-7 business working days.</p>
...
$objDemo = new \stdClass();
		...
		mail($admin_email, $admin_subject, $admin_msg, $headers);
	        ...
	        //$msg = "Exchange Added Successfully.";
            //return response()->json($msg);
			return response()->json(array('redirect_url' => route('admin-exchange-show',$Exchange->id)));
			}
    ...
    {
		return view('admin.exchange.open');
	}
	
	public function openexchange_datatables()
    {
         
		$datas = Exchange::where('status','=','pending')->orderBy('id','desc')->get();        
         ...
         return Datatables::of($datas)
		                     ...
		                     ->editColumn('vendor_id',function(Exchange $data){
                                $users = User::find($data->vendor_id);
                                return  $users->name;
                            }) 
                            ->editColumn('order_id',function(Exchange $data){
                       ...
                       ->editColumn('action', function(Exchange $data) {								
                                ...
                                <div class="action-list"><a href="'.route('admin-exchange-show',$data->id).'"><i class="fa fa-eye"></i>Details</a>
									 </div>
                                </div>';
								return $action;
    ...
    {
		return view('admin.exchange.ship');
	}
	
	public function shipexchange_datatables()
    {
         
		$datas = Exchange::where('status','=','shipped')->orderBy('id','desc')->get();        
         ...
         return Datatables::of($datas)
                          ...
                          ->editColumn('product_sku',function(Exchange $data){
                           ...
                           ->editColumn('courier_name',function(Exchange $data){
                                $courier_name = $data->companyname;
                                return  $courier_name;
                            }) 
                           ->editColumn('track_code',function(Exchange $data){
                                $track_code = $data->title;
                                return  $track_code;
                            }) 
                          ->editColumn('track_url',function(Exchange $data){
                                $track_url = $data->text;
                                return  $track_url;
                            })  							
                           ->editColumn('created_at', function(Exchange $data) {
							...
							->editColumn('action', function(Exchange $data) {								
								$action = '<a href="javascript:;" data-href="'. route('admin-exchange-loads',$data->id) .'" class="exchange-accept" data-toggle="modal" data-target="#modalaccept">Accept</a> <a href="javascript:;" data-href="'. route('admin-exchange-nloads',$data->id) .'" class="exchange-del" data-toggle="modal" data-target="#modaldel">Notdelivered</a>';
								return $action;
    ...
    {
		return view('admin.exchange.resolved');
	}
	public function resolved_datatables()
    {
         
		$datas = Exchange::where('status','=' ,'delivered')->orderBy('id','desc')->get();        
    ...
    public function notdeliveredexchange()
    {
		return view('admin.exchange.notdeliveredexchange');
	}
    ...
    public function notdelivered_datatables()
    {
		...
		$datas = Exchange::where('status','=' ,'notdelivered')->orderBy('id','desc')->get();        
    ...
    {
		return view('admin.exchange.decline');
	}
	public function decline_datatables()
    {
         
		$datas = Exchange::where('status','=','decline')->orderBy('id','desc')->get();        
	...
	public function show($id)
    {
		...
		return view('admin.exchange.details',compact('data'));
	...
	public function dstatus(Request $request, $id)
    {
        ...
        $dispute->update();
		$product_data = Product::findOrFail($product_id);
		$newmsg = 'Exchange Request Decline'; 
        $newmsg .= '<br> Product Name : ' . $product_data->name.'( '.$product_data->sku.' )';		
...
if($prod['size']){
   ...
   if($gs->is_smtp == 1)
       ...
       // return response()->json($msg);
		//return response()->json(array('redirect_url' => route('admin-exchange-show',['id'=>$dispute->id])));
        return response()->json(array('redirect_url' => route('admin-decline-exchange')));
	}
	public function loads($id)
    {
		$data = Exchange::where('id','=',$id)->first();
		return view('admin.exchange.load',compact('data'));
		
	}
	public function nloads($id)
    {
		$data = Exchange::where('id','=',$id)->first();
		return view('admin.exchange.nload',compact('data'));
    ...
    {
 		$data = Exchange::where('id','=',$id)->first();
 		return view('admin.exchange.notdelivered',compact('data'));
...
$companyname= $dispute->companyname;
	DB::table('vendor_orders')->where(['order_id' => $oid,'product_id' => $product_id,'user_id' => $vid])->update(['other_status' => '','exchange_qty' => $product_quantitys,'ex_status' => $ex_status]);		
		$dispute->update();
		$product_data = Product::findOrFail($product_id);
		$newmsg = 'Exchange Request Not Delivered';
        $newmsg .= '<br> Product Name : ' . $product_data->name.'( '.$product_data->sku.' )';		
		 if(!empty($dispute->tracking_code)){
            $newmsg .='<br> Code : '. $dispute->tracking_code;
        }else{
                                             ...
                                             <tbody>
                                                <tr>
                                                   <td valign="top" style="padding:48px 48px 32px">
                                                      <div style="color:#717a7a;font-size:14px;line-height:150%;text-align:left">
                                                         <p style="margin:0 0 16px">Hi '.$user_info->name.',</p>
                                                         <p style="margin:0 0 16px">We have initiated the exchange for your order #'.$order_number.'. It will be not delivered.</p>
                                                         <h2 style="color:#111111;display:block;font-size:18px;font-weight:bold;line-height:130%;margin:0 0 18px;text-align:left">
                                                            <a href="https://shop.southindiajewels.com/demo/wp-admin/post.php?post=10439&amp;action=edit" style="font-weight:normal;text-decoration:underline;color:#111111" target="_blank">[<span class="il">Order</span> #'.$order_number.']</a> ('.date('M d,Y',strtotime($order->created_at)).')
                                                         </h2>
                                                         <div style="margin-bottom:40px">
...
if($prod['size']){
     $vendor_msg .='<br /><strong>Option :</strong>'.str_replace('-',' ',$prod['size']).'   </td>';
                             ...
                             $vendor_msg .='</tbody>
                                                         ...
                                                         $vendor_msg .='<tr>
                                                            <th scope="row" colspan="2" style="color:#717a7a;border:1px solid #e4e4e4;vertical-align:middle;padding:12px;text-align:left;border-top-width:4px">Order Status :</th>
                                                            <td style="color:#717a7a;border:1px solid #e4e4e4;vertical-align:middle;padding:12px;text-align:left;border-top-width:4px"><span><span>Exchange Not Delivered</td>
                                                         </tr>';
...
if($prod['size']){
 ...
 //	public function notdelivered($id)
   // {
 	//	$data = Exchange::where('id','=',$id)->first();
 	//	return view('admin.exchange.notdeliverd',compact('data'));
...
// 	}
	...
	DB::table('vendor_orders')->where(['order_id' => $oid,'product_id' => $product_id,'user_id' => $vid])->update(['other_status' => 'exchange','exchange_qty' => $product_quantity,'ex_status' => $ex_status]);		
		...
		$product_data = Product::findOrFail($product_id);
		$newmsg = 'Exchange Request Delivered';
        $newmsg .= '<br> Product Name : ' . $product_data->name.'( '.$product_data->sku.' )';		
		 if(!empty($dispute->tracking_code)){
            $newmsg .='<br> Code : '. $dispute->tracking_code;
        }else{
		...
		$order_number = $order->order_number;	
			$cart = unserialize(bzdecompress(utf8_decode($order->cart)));			
			$to = $order->customer_email;	
            $subject = 'Your South India Jewels order '.$order->order_number.' is Exchange';
...
$vendor_em = 'vendor@southindiajewels.co.in';
//$vendor_em = $user_info->email;
$vendor_subject = 'Exchange For Your South India Jewels order '.$order->order_number.'';
                                             ...
                                             <tbody>
                                                <tr>
                                                   <td valign="top" style="padding:48px 48px 32px">
                                                      <div style="color:#717a7a;font-size:14px;line-height:150%;text-align:left">
                                                         <p style="margin:0 0 16px">Hi '.$order->customer_name.',</p>
                                                         <p style="margin:0 0 16px">We have initiated the exchange for your order #'.$order_number.'. It will be credited in 5-7 business working days.</p>
                                                         <h2 style="color:#111111;display:block;font-size:18px;font-weight:bold;line-height:130%;margin:0 0 18px;text-align:left">
                                                            <a href="https://shop.southindiajewels.com/demo/wp-admin/post.php?post=10439&amp;action=edit" style="font-weight:normal;text-decoration:underline;color:#111111" target="_blank">[<span class="il">Order</span> #'.$order_number.']</a> ('.date('M d,Y',strtotime($order->created_at)).')
                                                         </h2>
                                                         <div style="margin-bottom:40px">
...
if($prod['size']){
   ...
   if($gs->is_smtp == 1)
       ...
       $msg = 'Exchange order accept successfully';
       return response()->json($msg);		
	}  
...
$dispute->update();
 return response()->json(array('redirect_url' => route('admin-exchange-show',['id'=>$id])));
}else{
	return response()->json(array('errors' => 'Please Upload Document'));
}
}	

public function allexchangeExcelFile(Request $request){
	$all_datas = Exchange::orderBy('id','desc')->get();
	...
	$headers = array(
		...
		$columns = array('SL No','Vendor Name','Order Id','Product Name','Product SKU','Quantity','Customer Courier Partner','Customer Tracking Code','Customer Tracking Url','Courier Name','Tracking Code','Tracking URL','Reason','Exchange Date');
        ...
        $file = fopen('php://output', 'w');
         fputcsv($file, $columns);		 
            foreach ($all_datas as $all_data) {	
			  ...
			  $courier_partner = $all_data->courier_partner;
			  $tracking_code = $all_data->tracking_code;
			  $tracking_url = $all_data->tracking_url;
			  ...
			  $row['Customer Courier Partner']    = $courier_partner;
			  $row['Customer Tracking Code']    = $tracking_code;
			  $row['Customer Tracking Url']    = $tracking_url;
			  $row['Courier Name']    = $companyname;
			  $row['Tracking Code']    = $title;
			  $row['Tracking URL']    = $text;			  
			  $row['Reason']    =   $reason;			  
			  $row['Exchange Date']    = $created_at;			  
			  fputcsv($file, array($row['SL No'],$row['Vendor Name'],$row['Order Id'],$row['Product Name'],$row['Product SKU'],$row['Quantity'],$row['Customer Courier Partner'],$row['Customer Tracking Code'],$row['Customer Tracking Url'],$row['Courier Name'],$row['Tracking Code'],$row['Tracking URL'],$row['Reason'],$row['Exchange Date'] ));
				$j++;
			}			
            fclose($file);
        };	
