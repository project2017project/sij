...
class DebitNoteController extends Controller
{
    ...
    public function index(){
        ...
        return view('admin.debitnote.index',compact('users'));
    } 
	 public function load($id)
    {
        $order = Order::whereRaw("find_in_set($id , vendor_id_list)")->orderBy('id','desc')->get();
        return view('admin.debitnote.order',compact('order'));
    }
	 public function pload($id)
    {
        $product = Product::where('id','=',$id)->orderBy('id','desc')->get();
        return view('admin.debitnote.product',compact('product'));
    }
    	 public function ptload($id)
    {
        $product = Product::where('id','=',$id)->orderBy('id','desc')->get();
        return view('admin.debitnote.taxes',compact('product'));
    }
        ...
        if ($validator->fails()) {
          return response()->json(array('errors' => $validator->getMessageBag()->toArray()));
            ...
            $vendor_id = $request->vendor_id;			
	         return response()->json(array('redirect_url' => route('admin-add-debitnote',['orderid'=>$order_id,'vendorid'=>$vendor_id])));	 
	...
	public function add($orderid,$vendorid)
    {
		...
		return view('admin.debitnote.add',compact('vdata','vendordata','orderid','vendorid'));
	}
	...
	public function addstore(Request $request)
    {
        ...
        if($files=$request->file('screenshot')){
			...
			$DebitNote->fill($input)->save();
            $m_id=$DebitNote->id;
            $order_id=$DebitNote->order_id;
			$user_id=$DebitNote->vendor_id;
			$product_id=$DebitNote->product_id;
			$product_quantity=$DebitNote->quantity;
			$product_amount=$DebitNote->amount;
			
			$product_data = Product::findOrFail($product_id);
			
			$newmsg = 'Debit Note Request Created Reason is ' . $DebitNote->reason; 
			...
			$notification->vid = $DebitNote->vid;
            ...
            $subject = 'Your South India Jewels order '.$order->order_number.' request for DebitNote';
...
$vendor_subject = 'DebitNote Request For Your South India Jewels order '.$order->order_number.'';
                                 ...
                                 <tbody>
                                    <tr>
                                       <td style="padding:36px 48px;display:block">
                                          <h1 style="font-size:30px;font-weight:300;line-height:150%;margin:0;text-align:left;color:#ffffff"><span class="il"></span> <span class="il">Request DebitNote Order</span>: #'.$order_number.'</h1>
                                       </td>
                                    </tr>
                                 </tbody>
                              </table>
                                                ...
                                                <tr>
                                                   <td valign="top" style="padding:48px 48px 32px">
                                                      <div style="color:#717a7a;font-size:14px;line-height:150%;text-align:left">
                                                         ...
                                                         <p style="margin:0 0 16px">We have reuested the debit note for your order #'.$order_number.'. It will be credited in 5-7 business working days.</p>
...
if($prod['size']){
     $msg .='<br /><strong>Option :</strong>'.str_replace('-',' ',$prod['size']).'   </td>';
                             ...
                             $msg .='</tbody>
                                                               <tfoot>
                                                                  <tr>
                                                                     <th scope="row" colspan="2" style="color:#717a7a;border:1px solid #e4e4e4;vertical-align:middle;padding:12px;text-align:left;border-top-wi
																	 dth:4px">Subtotal ('.$product_quantity.' items):</th>
                                                                     <td style="color:#717a7a;border:1px solid #e4e4e4;vertical-align:middle;padding:12px;text-align:left;border-top-width:4px"><span><span>'.$order->currency_sign.'</span>'.$product_amount.'</span></td>
                                                                  </tr>';
...
$objDemo = new \stdClass();
        ...
        //mail($to, $subject, $msg, $headers);
			...
			return response()->json(array('redirect_url' => route('admin-debitnote-show',$DebitNote->id)));
			}else{
	...
	public function opendebit()
    {
		return view('admin.debitnote.open');
	}
	
	public function opendebit_datatables()
    {
         
		$datas = DebitNote::where('status','=',0)->orderBy('id','desc')->get();        
         ...
         return Datatables::of($datas)
		                     ...
		                     ->editColumn('vendor_id',function(DebitNote $data){
                                $users = User::find($data->vendor_id);
                                return  $users->name;
                            }) 
                            ->editColumn('order_id',function(DebitNote $data){
                       ...
                       ->editColumn('action', function(DebitNote $data) {								
                                ...
                                <div class="action-list"><a href="'.route('admin-debitnote-show',$data->id).'"><i class="fa fa-eye"></i>Details</a>
									 </div>
                                </div>';
								return $action;
    ...
    {
		return view('admin.debitnote.resolved');
	}
	public function resolved_datatables()
    {
         
		$datas = DebitNote::where('status','=',1)->orderBy('id','desc')->get();        
         ...
         return Datatables::of($datas)
                          ...
                          ->editColumn('product_sku',function(DebitNote $data){
                           ...
                           ->editColumn('created_at', function(DebitNote $data) {
                            ...
                            ->editColumn('action', function(DebitNote $data) {								
								$action = '<div class="godropdown"><button class="go-dropdown-toggle"> Actions<i class="fas fa-chevron-down"></i></button>
                                <div class="action-list"><a href="'.route('admin-debitnote-cancelled',$data->id).'"><i class="fa fa-eye"></i>Details</a>
									 </div>
                                </div>';
								return $action;
    ...
    {
		return view('admin.debitnote.decline');
	}
	public function decline_datatables()
    {
         
		$datas = DebitNote::where('status','=',2)->orderBy('id','desc')->get();        
	...
	public function show($id)
    {
		...
		return view('admin.debitnote.details',compact('data'));
		
	}
	public function cancelled($id)
    {
		$data = DebitNote::where('id','=',$id)->first();
		return view('admin.debitnote.canceldetails',compact('data'));
...
public function rstatus(Request $request, $id)
    {
        ...
        $debitnote->status = 1;
        $debitnote->update();
		$oid=$debitnote->order_id;
	    $vid=$debitnote->vendor_id;
	    $product_id=$debitnote->product_id;
		$product_data = Product::findOrFail($product_id);
		$newmsg = 'Debit Note Request is Resolved '; 
		$newmsg .= '<br> Product Name : ' . $product_data->name.'( '.$product_data->sku.' )';
		$product_data = Product::findOrFail($product_id);
			$notification = new Notification;
...
public function cancelstatus(Request $request, $id)
    {
	    ...
	    $product_id=$debitnote->product_id;
        $product_quantity=$debitnote->quantity;
        $product_reason=$debitnote->reason;
		
        $debitnote->status = 2;
        $debitnote->update();
		$product_data = Product::findOrFail($product_id);
		$newmsg = 'Debit Note Request is Cancel ';
        $newmsg .= '<br> Product Name : ' . $product_data->name.'( '.$product_data->sku.' )'; 		
			$notification = new Notification;
			$notification->order_id = $oid;			
			$notification->vendor_id = $vid;
		...
		$order_number = $order->order_number;	
			$cart = unserialize(bzdecompress(utf8_decode($order->cart)));			
			$to = $order->customer_email;	
            $subject = 'Your South India Jewels Order '.$order->order_number.' Debit Note Cancel';
...
$vendor_em = 'vendor@southindiajewels.co.in';
//$vendor_em = $user_info->email;
$vendor_subject = 'Debit Note For Your South India Jewels Order '.$order->order_number.'is Cancel';
...
public function export($status){    		 
	...
	$datas = DebitNote::orderBy('id','desc')->get();	
	$fileName = 'debit.csv';	
...
if($document) {
$dispute = DebitNote::find($id);
$dispute->document = implode(",",$document);
$dispute->update();
 return response()->json(array('redirect_url' => route('admin-debitnote-show',['id'=>$id])));
}else{
	return response()->json(array('errors' => 'Please Upload Document'));
}
}

public function alldebitExcelFile(Request $request){
	$all_datas = DebitNote::orderBy('id','desc')->get();
	$fileName = 'debit_all.csv';	
	...
	$headers = array(
		...
		$columns = array('SL No','Vendor Name','Order Id','Product Name','Product SKU','Total Deduction Amount','Quantity','IS PAYMENT','SGST','CGST','TCS','ADMIN FEE','AMT BEFORE TAX','OTHER AMT','REMARKS','Reason','Withdraw ID','Debit Date');
        ...
        $file = fopen('php://output', 'w');
         fputcsv($file, $columns);		 
            foreach ($all_datas as $all_data) {	
			  ...
			  $is_payment = $all_data->is_payment;
			  $sgst = $all_data->sgst;
			  $cgst = $all_data->cgst;
			  $igst = $all_data->igst;
			  $tcs = $all_data->tcs;
			  $adminfee = $all_data->adminfee;
			  $amt_before_tax = $all_data->amt_before_tax;
			  $others_amt = $all_data->others_amt;
			  ...
			  $row['Total Deduction Amount']    = $amount;
			  $row['Quantity']    = $quantity;
			  $row['IS PAYMENT']    = $is_payment;
			  $row['SGST']    = $sgst;
			  $row['CGST']    = $cgst;
			   $row['IGST']    = $igst;
			  $row['TCS']    = $tcs;
			  $row['ADMIN FEE']    = $adminfee;
			  $row['AMT BEFORE TAX']    = $amt_before_tax;
			  $row['OTHER AMT']    = $others_amt;
			  $row['REMARKS']    = $remarks;
			  $row['Reason']    =   $reason;
			  $row['Withdraw ID']    = $withdraw_id;
			  $row['Debit Date']    = $created_at;			  
			  fputcsv($file, array($row['SL No'],$row['Vendor Name'],$row['Order Id'],$row['Product Name'],$row['Product SKU'],$row['Total Deduction Amount'],$row['Quantity'],$row['IS PAYMENT'],$row['SGST'],$row['CGST'],$row['IGST'],$row['TCS'],$row['ADMIN FEE'],$row['AMT BEFORE TAX'],$row['OTHER AMT'],$row['REMARKS'], $row['Reason'],$row['Withdraw ID'],$row['Debit Date'] ));
				$j++;
			}			
            fclose($file);
        };	
