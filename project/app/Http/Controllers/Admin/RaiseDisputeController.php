...
$objDemo = new \stdClass();
	...
	public function opendispute_datatables()
    {
        ...
        $datas=$query->where('status','=','open')->orderBy('id','desc')->get();		        
...
if($prod['size']){
   ...
   if($gs->is_smtp == 1)
        ...
        //return response()->json($msg);
		//return response()->json(array('redirect_url' => route('admin-dispute-addshow',['id'=>$dispute->id])));

return response()->json(array('redirect_url' => route('admin-resolved-dispute')));
...
if($prod['size']){
   ...
   if($gs->is_smtp == 1)
       ...
       // mail($to, $subject, $msg, $headers);
		...
		//return response()->json($msg);
		//return response()->json(array('redirect_url' => route('admin-dispute-addshow',['id'=>$dispute->id])));
        return response()->json(array('redirect_url' => route('admin-decline-dispute')));
...
public function allraisedisputeExcelFile(Request $request){
	$all_datas = RaiseDispute::orderBy('id','desc')->get();
	$fileName = 'raise_dispute.csv';	
	...
	$headers = array(
		...
		$columns = array('SL No','Vendor Name','Order Id','Product Name','Product SKU','Total Amount','Quantity','Reason','Return Courier Partner','Return Tracking Code','Return Tracking Url','Dispute Date');
        ...
        $file = fopen('php://output', 'w');
         fputcsv($file, $columns);		 
            foreach ($all_datas as $all_data) {	
			  ...
			  $tracking_partner = $all_data->tracking_partner;
			  ...
			  $row['Reason']    =   $reason;
			  $row['Return Courier Partner']    = $tracking_partner;
			  $row['Return Tracking Code']    = $tracking_code;
			  $row['Return Tracking Url']    = $tracking_url;			  
			  $row['Dispute Date']    = $created_at;			  
			  fputcsv($file, array($row['SL No'],$row['Vendor Name'],$row['Order Id'],$row['Product Name'],$row['Product SKU'],$row['Total Amount'],$row['Quantity'], $row['Reason'],$row['Return Courier Partner'],$row['Return Tracking Code'],$row['Return Tracking Url'],$row['Dispute Date'] ));
				$j++;
			}			
            fclose($file);
        };	
