			...
			<div class="row">
				<div class="col-lg-12">
					...
					<ul class="links">
						...
						<li><a href="{{ route('vendor-prod-index') }}">{{ __("All Products") }}</a></li>
					</ul>
				</div>
			</div>
		</div>
			...
			<div class="col-lg-12">
				<div class="mr-table allproduct">
					...
					<div class="table-responsiv">
                        ...
                        <br>
					    <form id="geniusform" class="form-horizontal" action="{{route('admin-vendorprod-price-update')}}" method="POST" enctype="multipart/form-data">      
						    {{ csrf_field() }}
							<table id="geniustable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
								<thead>
									<tr>
				                        <th><input type="checkbox"  id="checkAll">{{ __("Product Image") }}</th><th>{{ __("Name") }}</th><th>{{ __("SKU") }}</th>
				                        <th>{{ __("Stock") }}</th><th>{{ __("Price") }}</th><th>{{ __("Categories") }}</th>
				                        <th>{{ __("Date") }}</th>
...
{{-- DATA TABLE --}}
    <script type="text/javascript">
		var table = $('#geniustable').DataTable({
               ...
               serverSide: true,
               ajax: {
                  url: "{{ route('vendor-prod-datatables') }}",
                  type: 'GET',
                  data: function (d) {
                    d.category         = $('#cat').val();
                    d.subcat           = $('#subcat').val();
       ...
       $(function() {
            $(".btn-area").append('<div class="col-sm-2 table-contents">'+
              '<a class="add-btn" href="{{route('vendor-prod-vproductupdate')}}">'+
              '<i class="fas fa-plus"></i> <span class="remove-mobile">{{ __("Product Update") }}<span>'+
              '</a>'+
              '</div>');
        });
