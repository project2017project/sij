...
class CustomenquiryController extends Controller
{
    ...
    {
         $datas = CustomEnquiry::orderBy('id','desc')->get();

         //--- Integrating This Collection Into Datatables
         return Datatables::of($datas)
                            ->editColumn('id', function(CustomEnquiry $data) {
                                $price = $data->id;
                                return  $price;
                            })
                            ->editColumn('description', function(CustomEnquiry $data) {
                                $description = $data->address;
                                return  $description;
                            })
                                                                                   
                            ->editColumn('name', function(CustomEnquiry $data) {
                                $name = mb_strlen(strip_tags($data->name),'utf-8') > 50 ? mb_substr(strip_tags($data->name),0,50,'utf-8').'...' : strip_tags($data->name);
                                
                                $id2 = '<small class="ml-2"> <b>Phone Number:</b>'.$data->phonenumber.'</small>';
                                $id3 = '<small class="ml-2"> <b>Email : </b>'.$data->email.'</small>';
                                $metal = '<small class="ml-2"><b> Metal Type : </b>'.$data->metal.'</small>';
                                $productCategory = '<small class="ml-2"> <b>Product Category : </b>'.$data->hueofmetal.'</small>';
                                $hueofmetal = '<small class="ml-2"> <b>hue of metal : </b>'.$data->hueofmetal.'</small>';
                                $gemstonepreference = '<small class="ml-2"> <b>gem stone preference : </b>'.$data->gemstonepreference.'</small>';
                                 $tentativebudget = '<small class="ml-2"> <b>tentative budget : </b>'.$data->datadealing.'</small>';
                                 $datadealing = '<small class="ml-2"> <b>Date : </b>'.$data->datadealing.'</small>';
                                  $linkoftheitem = '<small class="ml-2"> <b>URL : </b><a href="'.$data->linkoftheitem.'">'.$data->linkoftheitem.'</a></small>';

                                // $id3 = $data->type == 'Physical' ?'<small class="ml-2"> SKU: <a href="'.route('front.product', $data->slug).'" target="_blank">'.$data->sku.'</a>' : '';
                                return  $name.'<br>'.$id2.'<br>'.$id3.'<br>'.$metal.'<br>'.$productCategory.'<br>'.$hueofmetal.'<br>'.$gemstonepreference.'<br>'.$tentativebudget.'<br>'.$datadealing.'<br>'.$linkoftheitem;
                            })
                            ->addColumn('action', function(CustomEnquiry $data) {
                                ...
                                return '<div class="godropdown"><button class="go-dropdown-toggle"> Actions<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i> View Gallery</a><a href="javascript:;" data-href="' . route('admin-customenquiry-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> Delete</a></div></div>';
                            })
                            ->rawColumns(['name','details', 'status', 'action'])
    ...
    {
        return view('admin.customenquiry.index');
    }

    //*** GET Request
    public function deactive()
    {
        return view('admin.customenquiry.deactive');
    }

    //*** GET Request
    public function catalogs()
    {
        return view('admin.customenquiry.catalog');
    }

    //*** GET Request
    public function types()
    {
        return view('admin.customenquiry.types');
    }
    ...
    public function createPhysical()
    {
        $cats = Category::all();
        $sign = Currency::where('is_default','=',1)->first();
        return view('admin.customenquiry.create.physical',compact('cats','sign'));
    }
    ...
    public function createDigital()
    {
        $cats = Category::all();
        $sign = Currency::where('is_default','=',1)->first();
        return view('admin.customenquiry.create.digital',compact('cats','sign'));
    }
    ...
    public function createLicense()
    {
        $cats = Category::all();
        $sign = Currency::where('is_default','=',1)->first();
        return view('admin.customenquiry.create.license',compact('cats','sign'));
    }
    ...
    public function import(){
        ...
        $cats = Category::all();
        $sign = Currency::where('is_default','=',1)->first();
        return view('admin.customenquiry.productcsv',compact('cats','sign'));
    }
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function destroy($id)
    {
        ...
        $data = CustomEnquiry::findOrFail($id);
        if($data->enquirygalleries->count() > 0)
        {
            foreach ($data->enquirygalleries as $gal) {
                    if (file_exists(public_path().'/assets/images/galleriesenquiry/'.$gal->photo)) {
                        unlink(public_path().'/assets/images/galleriesenquiry/'.$gal->photo);
                    }
                $gal->delete();
            }
        ...
        $data->delete();
        //--- Redirect Section
        $msg = 'Custom Enquiry Deleted Successfully.';
        return response()->json($msg);
        //--- Redirect Section Ends
