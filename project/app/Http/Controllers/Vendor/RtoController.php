...
class RtoController extends Controller
{
    ...
    public function datatables($status){
        ...
        $datas = Rto::where('vendor_id','=',$user->id)->orderBy('id','desc')->get();
        ...
        $datas=$query->orderBy('id','desc')->get();
         ...
         return Datatables::of($datas)
							...
							->editColumn('vendor_name', function(Rto $data) {
								$users = User::find($data->vendor_id);
                                return $users->name;
                            })
                            ->editColumn('order_data', function(Rto $data) {
                            ...
                            ->editColumn('status', function(Rto $data) {
								return $data->status;							
                            })			
                       ...
                       ->editColumn('action', function(Rto $data) {								
                                ...
                                <div class="action-list"><a href="'.route('vendor-rto-show',$data->id).'"><i class="fa fa-eye"></i>Details</a>
									 </div>
                                </div>';
								return $action;
    ...
    {
        $user = Auth::user();
        $Rto = Rto::where('vendor_id','=',$user->id)->orderBy('id','desc')->get();
		$pending__count = Rto::where('vendor_id','=',$user->id)->where('status','=','pending')->orderBy('id','desc')->count('id');
        return view('vendor.rto.index',compact('user','Rto','pending__count'));
    }
	public function show($id)
    {
		$data = Rto::where('id','=',$id)->first();
		return view('vendor.rto.details',compact('data'));
    ...
    $product_reason=$dispute->reason;	
		...
		$newmsg = 'Rto Request has been shipped'; 
        ...
        $subject = 'Your South India Jewels Rto order '.$order->order_number.' is Shipped';
		$admin_subject = 'Your South India Jewels Rto order '.$order->order_number.' is Shipped';
                                                ...
                                                <tr>
                                                   <td valign="top" style="padding:48px 48px 32px">
                                                      <div style="color:#717a7a;font-size:14px;line-height:150%;text-align:left">
                                                         ...
                                                         <p style="margin:0 0 16px">We have shipped the Rto for your order #'.$order_number.'.</p>
...
if($prod['size']){
 ...
 if($gs->is_smtp == 1)
	...
	$headers = array(
        ...
        $columns = array('Order No','Order Date','Vendor Name','Product Name', 'Product Sku', 'Quantity', 'Payment Status', 'Rto Date');
       	$j=1;
		$callback = function() use($datas, $columns,$j) {
         ...
         fputcsv($file, $columns);		 
            foreach ($datas as $data) {
                ...
                if(@$data->created_at){				
				$row['Rto Date']    = $data->created_at;
				}else{
					$row['Rto Date']    = '';
				}				
                fputcsv($file, array($row['Order No'],$row['Order Date'],$row['Vendor Name'],$row['Product Name'],$row['Product Sku'],$row['Quantity'],$row['Payment Status'],$row['Rto Date']));
				$j++;
