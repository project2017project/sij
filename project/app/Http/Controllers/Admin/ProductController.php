...
class ProductController extends Controller
{
    ...
    public function datatables()
    {
		  ...
		  $stock_val = (!empty($_GET["stock"])) ? ($_GET["stock"]) : ('');
		...
		if(!empty($category) || !empty($svendor)){	
			...
			$query = Product::where('product_type','=','normal');
					...
					if(!empty($svendor))
						...
						if($stock_val=='in')
			$query->where('sum_stock','>=',1);
		  elseif($stock_val=='out')
			$query->where('sum_stock','=',0);
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('image',function(Product $data){
                                $pricecheckbox= "<input type='checkbox' style='position:absolute;opacity:0;visibility:hidden;' name='pricebox[]' value='$data->price'  >";
                                 ...
                                 $rpricecheckbox= "<input type='checkbox' style='position:absolute;opacity:0;visibility:hidden;' name='rpricebox[]' value='$data->previous_price'  >";
                                ...
                                return $checkbox.$pricecheckbox.$rpricecheckbox.'<a href="'.route('front.product', $data->slug).'" target="_blank">'.$productimage.'</a>';
                            })
                            ...
                            ->editColumn('stock', function(Product $data) {
                                /*$stck = (string)$data->sum_stock;
                                if($stck == "0")
                                return "Out Of Stock";
                                elseif($stck == null)
                                return "0";
                                else
                                return $data->sum_stock;*/
	  ...
	  public function simple_datatables()
    {
		...
		if(!empty($category) || !empty($svendor)){	
			...
			$query = Product::where('product_type','=','normal')->whereNull('size');
					if(!empty($category))
						$query->whereRaw("find_in_set($category , category_id)");					
					if(!empty($subcat))
						$query->whereRaw("find_in_set($subcat , subcategory_id)");	
         ...
         return Datatables::of($datas)
                            ...
                            ->rawColumns(['name','image','vendor','price', 'date', 'sku', 'status', 'action'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }
	
	public function variation_datatables()
    {
         $category = (!empty($_GET["category"])) ? ($_GET["category"]) : ('');
          $subcat = (!empty($_GET["subcat"])) ? ($_GET["subcat"]) : ('');
          $childcat = (!empty($_GET["childcat"])) ? ($_GET["childcat"]) : ('');
		...
		if(!empty($category) || !empty($svendor)){	
			...
			$query = Product::where('product_type','=','normal')->where('size','!=','');
					if(!empty($category))
						$query->whereRaw("find_in_set($category , category_id)");					
					if(!empty($subcat))
						$query->whereRaw("find_in_set($subcat , subcategory_id)");	
            ...
            $datas = Product::where('product_type','=','normal')->where('size','!=','')->orderBy('id','desc')->get(); 
        }
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('image',function(Product $data){
                                ...
                                $url= asset('assets/images/thumbnails/'.$data->thumbnail);  
                                $productimage = "<img src=".$url.">";
                                return '<a href="'.route('front.product', $data->slug).'" target="_blank">'.$productimage.'</a>';
                            })
                            ->editColumn('sku',function(Product $data){
                                $sku = $data->type == 'Physical' ?'<span>  <a href="'.route('front.product', $data->slug).'" target="_blank">'.$data->sku.'</a></span>' : '';
                                return  $sku;
    ...
    public function simpleproduct()
    {
	 ...
	 //*** GET Request variationproduct
    public function variationproduct()
    {
        $cats = Category::all();
		$users = User::all()->where('is_vendor','2');
        return view('admin.product.variationproduct',compact('cats','users'));
    }
    ...
    public function store(Request $request)
    {
       ...
       if (empty($request->draft)){
        ...
        ob_start();
        return response()->json(array('redirect_url' => route('admin-prod-edit',$data->id)));
                        ...
                        if($chcat->exists()) {
                          ...
                          if($request->subcategory_id){
						   ...
						   if($request->childcategory_id){
							...
							$input['minPrice']           = $line[26];
							$input['maxPrice']           = $line[27];
       ...
       // if(!empty($request->checkedid)){
            ...
            if($request->gender=='s'){
                for($i=0;$i<=$count-1;$i++){
					...
					if($request->valuetype=='removeprice'){
                      $product_data = Product::findOrFail($request->checkedid[$i]);
					  $updateprice = $product_data->previous_price;
                    }
            ...
            if($request->gender=='r'){
                for($i=0;$i<=$count-1;$i++){
                    ...
                    if(!empty($addonprice)){
                        $updateprice = ($request->rpricebox[$i]+$addonprice);
                    }
                    if(!empty($addonpercentprice)){
                        $updateprice = ($request->rpricebox[$i]+ ($request->rpricebox[$i]*$addonpercentprice/100));
                    }
                    if(!empty($reduceonpercentprice)){
                        $updateprice = ($request->rpricebox[$i]- ($request->rpricebox[$i]*$reduceonpercentprice/100));
                    }
                    if(!empty($reducefixedprice)){
                        $updateprice = ($request->rpricebox[$i]-$reducefixedprice);
                    }
                    if($request->valuetype=='removeprice'){
                      $product_data = Product::findOrFail($request->checkedid[$i]);
					  $updateprice = $product_data->previous_price;
            ...
            if($request->stock=='instock'){
                for($i=0;$i<=$count-1;$i++){
                Product::where('id',$request->checkedid[$i])->update(['stock' => null,'sum_stock' => null]);
                }
            }
            if($request->stock=='outofstock'){
                for($i=0;$i<=$count-1;$i++){
                Product::where('id',$request->checkedid[$i])->update(['stock' => '0','sum_stock' =>'0']);
                }
            }
            if($request->stock=='stockupdate'){
                for($i=0;$i<=$count-1;$i++){
                    Product::where('id',$request->checkedid[$i])->where('size_qty','=',null)->update(['stock' => $request->staticstock,'sum_stock' => $request->staticstock]);
                }
            }
        // }else{
