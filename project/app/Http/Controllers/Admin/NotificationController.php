...
class NotificationController extends Controller
{
    ...
    {
        $datas = Notification::where('user_id','!=',null)->where('is_read','=',0)->latest('id')->get();
        if($datas->count() > 0){
    ...
    {
        $datas = Notification::where('order_id','!=',null)->where('is_read','=',0)->latest('id')->get();
        if($datas->count() > 0){
          foreach($datas as $data){
            $data->is_read = 1;
            $data->update();
    ...
    {
        $datas = Notification::where('product_id','!=',null)->where('is_read','=',0)->latest('id')->get();
        if($datas->count() > 0){
          foreach($datas as $data){
            $data->is_read = 1;
            $data->update();
    ...
    {
        $datas = Notification::where('conversation_id','!=',null)->where('is_read','=',0)->latest('id')->get();
        if($datas->count() > 0){
          foreach($datas as $data){
            $data->is_read = 1;
            $data->update();
