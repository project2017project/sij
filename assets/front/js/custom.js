...
$(document).on('change','.product-attr',function(){
         ...
         var attridprice = $(this).attr('data-price');
         
        var attridname = $(this).siblings('label').html();
        
        
        if (!$(".product-attr:checked").val()) {
  ...
  $('.product-addon-totals').show();
         ...
         $('.addon_li').append('<div id=' + attrid  + '><div style="float:left;" class="wc-pao-col1"><strong class="addon_text">' + attridname  + '</strong></div><div style="float:right;" class="wc-pao-col2"><span class="currsign">₹</span><strong class="addon_amount">'+attridprice + '</strong></div></div>');
         
         var priceproductfin = $('#calcprice').html();
         
          var priceproductfini = parseInt(priceproductfin);
          
          var a = 0;
    $(".addon_li .addon_amount").each(function() {
        a += parseInt($(this).html());
    });
          
          var attridpricei = parseInt(attridprice);
         
         
         
         
         var sumattr = priceproductfini + a;
           
                $(".subtotalamtli").html(sumattr);
           
         
         } else {
             
             
             $('.addon_li #'+attrid).remove();
             var priceproductfin = $('#calcprice').html();
         var priceproductfini = parseInt(priceproductfin);
             var a = 0;
    $(".addon_li .addon_amount").each(function() {
...
$(document).ready(function(){
    var qtyprod = $(".qttotal").html();
        
        var priceproductint = $('#sizeprice').html();
         
         var priceproductintc = priceproductint.replace(/[^0-9/.]/g,'')
         
         var finalpriceinit = qtyprod * priceproductintc;
        
        $("#calcprice").html(finalpriceinit);
        
        $(".subtotalamtli").html(finalpriceinit);
    ...
    $(document).on('click', '.qtminus', function () {
        ...
        var priceproductint = $('#sizeprice').html();
         ...
         var priceproductintc = Number(priceproductint .replace(/[^0-9\.]+/g,""));
         
         var priceproductintcdec = priceproductintc.toFixed(2);
         
         var finalpriceinit = qtyprod * priceproductintcdec;
        
        $("#calcprice").html(finalpriceinit);
        
        var attridprice = $(".addon_amount").html();
         ...
         var sumattr = priceproductfini + attridpricei;
    ...
    $("#coupon-form").on('submit', function () {
        ...
        var id = $("#user_id").val();
            ...
            $.ajax({
                    ...
                    data:{code:val, total:total,id:id},
                    success:function(data){
                        if(data == 0)
                        {
          ...
          $.ajax({
           ...
           success:function(data)
           {
              ...
              $('.alert-danger ul').html('');
                ...
                $('#refundform textarea').eq(0).focus();
              ...
              else
              {
                ...
                $('#star-rating').html(data[1]);
                $('#refundform textarea').eq(0).focus();
                $('#refundform textarea').val('');
                $('#refund-section').load($this.data('href'));
              }
              $('.gocover').hide();
              $('button.submit-btn').prop('disabled',false);
           }
  ...
  $('.submit-loader').removeClass('d-none');
      $.ajax({
       ...
       success:function(data)
       {
		   if(data == "success")
          {
			  $('.sucess').html("Message Sent Sucessfully.");
              $('.submit-loader').addClass('d-none');
			  window.setTimeout(function(){location.reload()},3000)
          }else{
			  $('.error').html("Message Unsent.");
              $('.submit-loader').addClass('d-none');
...
$(document).on('submit','#sendinvoice',function(e){	
e.preventDefault();
$('.submit-loader').removeClass('d-none');
var invoicedata = new FormData(this);
var sendinvoice = $(this);
    $.ajax({
     method:"POST",
     url:$(this).prop('action'),
     data:invoicedata,
...
$(document).on('click','.filter_btn_mobile',function(e){
    $(".fiter-box_side").toggleClass('show');
});


$(document).on('click','.close_filter_mobile',function(e){
    $(".fiter-box_side").toggleClass('show');
});



if ($("body").find(".fiter-box_side").length > 0){ 
  $(".shop-toolbar").addClass('w50s');
}
