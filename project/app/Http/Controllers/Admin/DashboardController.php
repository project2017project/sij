...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        for($i = 0; $i < 30; $i++) {
            ...
            $sales .=  "'".Order::where('status','!=','Pending')
        ...
        for($i = 6; $i > -1; $i--) {
            $days7 .= "'".date("d M", strtotime('-'. $i .' days'))."',";
            $sales7 .=  "'".Order::where('status','!=','Pending')
                                ->where('status','!=','declined')
                                ->whereDate('created_at', '=', date("Y-m-d", strtotime('-'. $i .' days')))
                                ->count()
                            ."',";
        ...
        return view('admin.dashboard',compact('pending','activation_notify','processing','completed','products','users','blogs','days','days7','sales','sales7','pproducts','rorders','poproducts','rusers','referrals','browsers'));
    }
