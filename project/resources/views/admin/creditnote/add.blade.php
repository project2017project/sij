...
<div class="content-area">
	<div class="mr-breadcrumb">
		<div class="row">
			<div class="col-lg-12">
				<h4 class="heading">{{ __('Credit Note') }} 				
				<ul class="links">
					<li><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }} </a></li>					
					<li><a href="javascript:;">{{ __('Add Credit Note') }}</a></li>					
				</ul>
			</div>
		</div>
	</div>

	<form id="addcreditnoteform" action="{{route('addstore-creditnote-submit')}}" method="POST" enctype="multipart/form-data">
		{{csrf_field()}}
 ...
 @endforeach
                                    ...
                                    @foreach($vdata as $vdatas)
									 @php
                                                ...
                                                @endphp
                                    <option data-href="{{ route('product-credit-pload',$vdatas->product_id) }}" value="{{$vdatas->product_id}}"  data-hreftax="{{ route('product-data-ptload',$vdatas->product_id) }}" value="{{$vdatas->product_id}}"  dataidp=".setproduct{{$j}}">({{$productsku->sku}}) {{$productsku->name}}</option>
									@php $j++ @endphp
                                    @endforeach
					                    </select>
									...
									<div class="row">
										<div class="col-lg-12"><div class="left-area"><h4 class="heading">{{ __('Final Debit Amount') }}* </h4></div></div>
										<div class="col-lg-12"><input type="text" class="input-field finaldnamt" placeholder="{{ __('Final Debit Amount') }}" name="amount" required="" ></div>
									</div>
    ...
    background-color: #28a745; border: 1px solid transparent; border-color: #28a745;" type="submit">{{ __('Confirm') }}</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">{{ __('Cancel') }}</button>
												...
												<a href="javascript:;" class="btn btn-info" data-toggle="modal" data-target="#creditmod">{{ __('Add') }}</a>
													</div>
									</div>
							</div>
						</div>
