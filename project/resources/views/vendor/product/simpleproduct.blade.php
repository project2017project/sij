			...
			<div class="row">
				<div class="col-lg-12">
					...
					<ul class="links">
						...
						<li><a href="{{ route('vendor-prod-simpleproduct') }}">{{ __("Simple Products") }}</a></li>
					</ul>
				</div>
			</div>
		</div>
...
{{-- DATA TABLE --}}
    <script type="text/javascript">
		var table = $('#geniustable').DataTable({
               ...
               serverSide: true,
               ajax: {
                  url: "{{ route('vendor-prod-simple-datatables') }}",
                  type: 'GET',
                  data: function (d) {
                    d.category         = $('#cat').val();
                    d.subcat           = $('#subcat').val();
      	...
      	$(function() {
            $(".btn-area").append('<div class="col-sm-1 table-contents">'+'<a href="javascript:void(0)" class="add-btn">'+
              ' <span data-toggle="modal" data-target="#bulk-edit" class="">{{ __("Bulk Edit") }}<span>'+
              '</a>'+'</div>');
        });
