...
//*** POST Request
public function refundamt(Request $request)
    {
	...
	$refund = $payment->refund($refundData);
		...
		if($id &&  $product_id && $product_quantity && $product_price) {
		...
		DB::table('vendor_orders')->where(['order_id' => $id,'product_id' => $product_id])->update(['product_item_qty' => $product_quantity,'product_item_price' => $product_price,'reason' => $product_reason,'refund_status' => 'online','ref_status' => $ref_status]);		
		}
		}
		}
		$reason= $request->pr_count;
...
$admin_em = Pagesetting::find(1)->contact_email;
		...
		$order_number = $order->order_number;	
            ...
            $admin_subject = '[South India Jewels]: Refund Initiated For #'.$order->order_number;
...
//*** POST Request
public function refundmanualamt(Request $request)
    {
		$email=env('ADMIN_EMAIL');		 
        $admindata = Admin::where('email',$email)->first();
	    $id= $request->order_id;
		...
		$order = Order::findOrFail($id);
		$order_number = $order->order_number;	
			$cart = unserialize(bzdecompress(utf8_decode($order->cart)));		$admindata = Admin::where('email',$email)->first(); 
$admin_em = Pagesetting::find(1)->contact_email;	
			$to = $order->customer_email;
            $subject = 'Refund Initiated For Your South India Jewels order '.$order->order_number.'';
            $admin_subject = '[South India Jewels]: Refund Initiated For #'.$order->order_number;
