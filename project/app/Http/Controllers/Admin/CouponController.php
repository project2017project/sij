...
class CouponController extends Controller
{
   public function __construct()
    ...
    {
        return view('admin.coupon.searchresult');
    }
    ...
    public function store(Request $request)
    {
		...
		$code=$data->code;
		$cart_total_limit=$data->cart_total_limit;
		$user_used_count=$data->user_used_count;
		$types='';
		$type=$data->type;
		if($type==1){
			$types='Discount By Amount';
		}elseif($type==0){
			$types='Discount By Percentage';
		}
		$gift_voucher=$data->gift_voucher;
		$email_address=$data->email_address;
		if($gift_voucher && $email_address){
		    $gs = Generalsetting::findOrFail(1);
            $headers = "From: ".$gs->from_name."<".$gs->from_email.">";
			$subject = 'Coupon Code'; 
            $msg ='Code Detais is Following :- ';       
        if(!empty($code)){
            $msg .='<br> Code : '. $code;
        }else{
            $msg .='';
        }
        if(!empty($cart_total_limit)){    
            $msg .='<br>  Cart Total : '. $cart_total_limit;
        }else{
            $msg .='';
        }
        if(!empty($user_used_count)){    
             $msg .='<br>  Limit To Use Code : '. $user_used_count;
        }else{
            $msg .='';
        }
         if(!empty($types)){    
