...
class OrderrecordController extends Controller
{
	...
	public function record(Request $request)
    {
		...
		$pending_orders =  VendorOrder::where('status','=','pending')->whereBetween('created_at',[$startdate,$enddate])->distinct()->count('order_id');        
        $pending_amount =  VendorOrder::where('status','=','pending')->whereBetween('created_at',[$startdate,$enddate])->sum('price');
		
		$processing_orders =  VendorOrder::where('status','=','processing')->whereBetween('created_at',[$startdate,$enddate])->distinct()->count('order_id');        
        $processing_amount =  VendorOrder::where('status','=','processing')->whereBetween('created_at',[$startdate,$enddate])->sum('price');
		
		$declined_orders =  VendorOrder::where('status','=','declined')->whereBetween('created_at',[$startdate,$enddate])->distinct()->count('order_id');        
        $declined_amount =  VendorOrder::where('status','=','declined')->whereBetween('created_at',[$startdate,$enddate])->sum('price');
		
		$od_orders =  VendorOrder::where('status','=','on delivery')->whereBetween('created_at',[$startdate,$enddate])->distinct()->count('order_id');       
        $od_amount =  VendorOrder::where('status','=','on delivery')->whereBetween('created_at',[$startdate,$enddate])->sum('price');
		
		$completed_orders =  VendorOrder::where('status','=','completed')->whereBetween('created_at',[$startdate,$enddate])->distinct()->count('order_id');        
        $completed_amount =  VendorOrder::where('status','=','completed')->whereBetween('created_at',[$startdate,$enddate])->sum('price');
		
		 $all_orders = VendorOrder::where('status','=','completed')->whereBetween('created_at',[$startdate,$enddate])->orderBy('id','desc')->get();
              
		}else{
			
		$allorders =  VendorOrder::where('status','=','completed')->where('user_id','=',$vendor_id)->whereBetween('created_at',[$startdate,$enddate])->distinct()->count('order_id');        
        $pay_amount =  VendorOrder::where('status','=','completed')->where('user_id','=',$vendor_id)->whereBetween('created_at',[$startdate,$enddate])->sum('price');
        ...
        $all_orders = VendorOrder::where('user_id','=',$vendor_id)->where('status','=','completed')->whereBetween('created_at',[$startdate,$enddate])->orderBy('id','desc')->get();		
		}
		
		
		return view('admin.orderrecord',compact('users','days_between','vendor_id','activation_notify','allorders','pay_amount','refund_fee','pending_orders','pending_amount','processing_orders','processing_amount','declined_orders','declined_amount','od_orders','od_amount','completed_orders','completed_amount','startdate','enddate','all_orders'));
    }   
}
