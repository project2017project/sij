...
class Product extends Model
{
    ...
    protected $fillable = ['user_id','vendor_id_list','product_id_list','product_item_list','product_name_qty','category_id','product_type','affiliate_link','sku', 'subcategory_id', 'childcategory_id', 'attributes', 'name', 'photo','variation_title', 'size','size_qty','size_price','size_pre_price','size_image', 'color', 'details','price','previous_price','stock','policy','status', 'views','tags','featured','best','top','hot','latest','big','trending','sale','sal_status','features','colors','product_condition','ship','meta_tag','meta_description','youtube','type','file','license','license_qty','link','platform','region','licence_type','measure','discount_date','is_discount','whole_sell_qty','whole_sell_discount','catalog_id','slug','category_multi_id','subcategory_multi_id', 'childcategory_multi_id','short_details','colloction','designer','bribal','chokars','others','update_by','other_image','minPrice','maxPrice','sum_stock','sum_price'];
    ...
    public  function setCurrency() {
        ...
        if($gs->currency_format == 0){
            return $curr->sign.number_format($price, 2, '.', '');
        }
        else{
            return number_format($price, 2, '.', '').$curr->sign;
        }
    }
    public function showPriceNew() {
        $gs = Generalsetting::findOrFail(1);
        $price = $this->price;
        $pricemax = $this->price;
        ...
        $price = round(($price) * $curr->value,2);
        $pricemax = round(($pricemax) * $curr->value,2);
        if($gs->currency_format == 0){
            $min = $curr->sign.$price;
            $max = $curr->sign.$pricemax;
            if($min == $max){
              $final= $min;   
            }else{
              $final= $min.' - '.$max;   
