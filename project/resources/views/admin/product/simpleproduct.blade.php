			...
			<div class="row">
				<div class="col-lg-12">
					...
					<ul class="links">
						...
						<li><a href="{{ route('admin-prod-simpleproduct') }}">{{ __("Simple Products") }}</a></li>
					</ul>
				</div>
			</div>
		</div>
					...
					<div class="table-responsiv">	
					    ...
					    <form id="geniusform" class="form-horizontal" action="{{route('admin-prod-price-update')}}" method="POST" enctype="multipart/form-data">      
							...
							<table id="geniustable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
								<thead>
									<tr>
				                        <th><input type="checkbox"  id="checkAll">{{ __("Product Image") }}</th><th>{{ __("Name") }}</th><th>{{ __("SKU") }}</th>
				                        <th>{{ __("Stock") }}</th><th>{{ __("Price") }}</th><th>{{ __("Category List") }}</th>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <input type="radio" name="gender" value="s"  />Selling Price
                                            <input type="radio" name="gender" value="r"  />Original Price
                                            <select name="valuetype" class="form-control">
												...
												<option value="removeprice">Remove Price</option>
                                            </select>
                                            <div class="sale_price_inputs"><input type="text"  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" name="pricevalue" value"" placeholder="Enter Value"></div>
                                            <h4>Stock</h4>
                                            ...
                                            <input type="text"   oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  class="form-control"  name="staticstock" value"" placeholder="Enter Value">
                                            <hr>
                                        </div>
...
{{-- DATA TABLE --}}
    <script type="text/javascript">
		var table = $('#geniustable').DataTable({
               ...
               serverSide: true,
               ajax: {
                  url: "{{ route('admin-prod-simple-datatables') }}",
                  type: 'GET',
                  data: function (d) {
                    d.category         = $('#cat').val();
                    d.subcat           = $('#subcat').val();
